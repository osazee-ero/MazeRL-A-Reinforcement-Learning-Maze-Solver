...
import numpy as np
import math
import Reinforcement_learning as ql
...
class Qboard:
    ...
    def playGame(self):
        ...
        qsession = ql.Qlearn(self.goal_reward, self.goal_reward2, self.discount_reward, self.loss_reward, self.fire_reward,100,400,0.001,0.2)
    ...
    def newGame(self):
        rand_gen = np.random.randint(0, 54210)
        self.root.destroy()
        #board = Qboard()
    ...
    def drawBoard(self,gridsize,walls,fire,random_state):
       ...
       self.num_states = tk.Scale(self.play, label="num_states", from_=3, to=17, orient='horizontal', command=get_num_states)
...
board = Qboard()
states = board.drawBoard(5,0.1,0.1,8032)
